...
if __name__ == "__main__":
    ...
    try:
        ...
        if selected_camera['camera_type'] != 'realsense':
            logger.warning(
                "Selected camera is USB. Tracking GUI currently supports RealSense only, so only preview was run."
            )
            print(
                "Selected camera is USB. Tracking GUI currently supports RealSense only, "
                "so only preview was run."
            )
            sys.exit(0)

        recording_params['serial_number'] = selected_camera['serial_number']
        logger.info("Launching tracking GUI with serial=%s", recording_params['serial_number'])
